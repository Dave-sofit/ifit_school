...
class UserMessengerMng(ServiceObjectBase):
    ...
    async def get(self, objectDB=None, queryParams: QueryParamFields = None, noneable=False, **kwargs) -> UserMessengerOut:
        return await super().get(objectDB=objectDB, queryParams=queryParams, noneable=noneable, **kwargs)

    async def getList(self, queryParams: UserMessengerQueryParamFields = None, order_by: dict | None = None, **kwargs) -> List[
        UserMessengerOut]:
